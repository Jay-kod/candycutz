...
if (! function_exists('generateVerificationCode')) {
    function generateVerificationCode($pdo, $barberId)
    {
        ...
        $initials = '';
        foreach (explode(' ', trim($bName)) as $w) {
            if (! empty($w)) {
...
if ($method === 'PATCH' && $path === '/barber/my-status') {
    ...
    if ($currentStatus === 'suspended' || $currentStatus === 'pending_approval') {
        http_response_code(403);
        echo json_encode(['error' => 'You cannot change your status while your account is '.str_replace('_', ' ', $currentStatus)]);
        exit;
    }
    ...
    if ($newStatus && ! in_array($newStatus, $allowed)) {
        ...
        echo json_encode(['error' => 'You can only set your status to Active or Not Active']);
        exit;
    }
...
if ($method === 'POST' && $path === '/barber/notifications') {
    ...
    cc_notify_admin($pdo, [
        'sender_id' => $userId,
        'type' => 'general_update',
        'title' => 'Barber Broadcast Sent',
        'message' => sprintf('Barber sent a notification to all customers: "%s".', $title),
    ]);
...
if (strpos($path, '/barber/services') === 0) {
    ...
    if ($method === 'POST') {
        ...
        if (! $name || $price <= 0) {
            http_response_code(400);
            echo json_encode(['error' => 'Name and valid price required']);
            exit;
        }
...
if (strpos($path, '/barber/gallery') === 0) {
    ...
    if ($method === 'POST' && preg_match('/^\/barber\/gallery$/', $path)) {
        ...
        if (isset($_FILES['image'])) {
            if ($_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadDir.$filename)) {
                    ...
                    http_response_code(500);
                    echo json_encode(['error' => 'Failed to move uploaded file. Check permissions for: '.$uploadDir]);
                    exit;
                }
            } else {
                ...
                if ($errCode === UPLOAD_ERR_INI_SIZE) {
                    $errMsg = 'Image file is too large (exceeds server limit).';
                }
                http_response_code(400);
        ...
        if (! $image_url) {
            http_response_code(400);
            echo json_encode(['error' => 'Image file or URL required']);
            exit;
        }
...
if (strpos($path, '/barber/blog') === 0) {
    if ($method === 'GET') {
        $stmt = $pdo->prepare("SELECT p.*, COALESCE(NULLIF(p.author_display, ''), u.name) as author_name,
            ...
            FROM blog_posts p LEFT JOIN users u ON p.author_id = u.id ORDER BY p.created_at DESC");
        $stmt->execute();
    ...
    if (($method === 'PUT' || $method === 'POST') && preg_match('/^\/barber\/blog\/(\d+)$/', $path, $matches)) {
        ...
        if (isset($_FILES['image'])) {
            if ($_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                $errMsg = 'Image upload failed with error code: '.$errCode;
                if ($errCode === UPLOAD_ERR_INI_SIZE) {
                    $errMsg = 'Image file is too large.';
                }
                http_response_code(400);
                echo json_encode(['error' => $errMsg]);
                exit;
        ...
        if ($author_display === '') {
            ...
            $author_display = $nameStmt->fetchColumn() ?: 'Barber';
        }
    ...
    if ($method === 'POST' && $path === '/barber/blog') {
        ...
        cc_notify_admin($pdo, [
            'sender_id' => $userId,
            'type' => 'blog_update',
            'title' => 'Blog Post Created by Barber',
            'message' => sprintf('Barber published blog post "%s".', $title),
            'related_entity_id' => $blogId,
        ]);
    ...
    if ($method === 'POST' && preg_match('/^\/barber\/bookings\/(\d+)\/verify-payment$/', $path, $matches)) {
        ...
        if ($action === 'approve') {
            ...
            cc_notify_appointment_parties($pdo, (int) $appointmentId, [
                ...
                'barber' => [
                    'title' => 'Payment Verified',
                    'message' => 'You verified a customer payment and confirmed the booking.',
                ],
                'admin' => [
                    'title' => 'Payment Verified by Barber',
                    'message' => 'Barber has verified the payment and confirmed booking.',
                ],
            ]);
            ...
            cc_notify_appointment_parties($pdo, (int) $appointmentId, [
                ...
                'customer' => [
                    ...
                    'message' => 'Your payment receipt was rejected. The booking has been cancelled.',
                ],
                'barber' => [
                    'title' => 'Payment Rejected',
                    'message' => 'You rejected a customer payment receipt and cancelled the booking.',
                ],
                'admin' => [
                    'title' => 'Payment Rejected by Barber',
                    'message' => 'Barber has rejected the payment receipt and cancelled the booking.',
                ],
            ]);
...
if (strpos($path, '/barber/appointments') === 0) {
        ...
        try {
            ...
            cc_notify_appointment_parties($pdo, (int) $appointmentId, [
                ...
                'customer' => [
                    ...
                    'message' => 'Your booking has been confirmed by the barber.',
                ],
                'barber' => [
                    'title' => 'Booking Confirmed',
                    'message' => 'You confirmed a customer booking.',
                ],
                'admin' => [
                    'title' => 'Booking Confirmed by Barber',
                    'message' => 'Barber has confirmed the booking.',
                ],
            ]);
            ...
            $pdo->rollBack();
            http_response_code(500);
            echo json_encode(['error' => 'Failed to approve: '.$e->getMessage()]);
        }
        exit;
    }
        ...
        try {
            ...
            cc_notify_appointment_parties($pdo, (int) $appointmentId, [
                ...
                'customer' => [
                    ...
                    'message' => 'Your booking has been cancelled by the barber.',
                ],
                'barber' => [
                    'title' => 'Booking Cancelled',
                ...
                'admin' => [
                    ...
                    'message' => 'Barber has cancelled the booking.',
                ],
            ]);
            ...
            $pdo->rollBack();
            http_response_code(500);
            echo json_encode(['error' => 'Failed to cancel: '.$e->getMessage()]);
        }
        exit;
    }
    ...
    if ($method === 'PATCH' && preg_match('/^\/barber\/appointments\/(\d+)\/complete$/', $path, $matches)) {
        ...
        if (! $isWalkIn && ! empty($appointment['verification_code']) && strtoupper(trim($appointment['verification_code'])) !== strtoupper(trim($providedCode))) {
            http_response_code(400);
            echo json_encode(['error' => 'Invalid verification code provided. Please ask the customer for their code.']);
            exit;
        }
        ...
        try {
            ...
            cc_notify_appointment_parties($pdo, (int) $appointmentId, [
                ...
                'customer' => [
                    ...
                    'message' => 'Thank you! Your booking has been marked as completed.',
                ],
                'barber' => [
                    'title' => 'Booking Completed',
                    'message' => 'You marked a booking as completed.',
                ],
                'admin' => [
                    'title' => 'Booking Completed by Barber',
                    'message' => 'Barber has marked booking as completed.',
                ],
            ]);
            ...
            $pdo->rollBack();
            http_response_code(500);
            echo json_encode(['error' => 'Failed to complete: '.$e->getMessage()]);
        }
        exit;
    }
        ...
        try {
            ...
            cc_notify_appointment_parties($pdo, (int) $appointmentId, [
                ...
                'customer' => [
                    ...
                    'message' => 'Your appointment was marked as a no-show.',
                ],
                'barber' => [
                    'title' => 'No-Show Recorded',
                    'message' => 'You marked a booking as no-show.',
                ],
                'admin' => [
                    'title' => 'Booking No-Show by Barber',
                    'message' => 'Barber has marked booking as no-show.',
                ],
            ]);
            ...
            $pdo->rollBack();
            http_response_code(500);
            echo json_encode(['error' => 'Failed to mark as no-show: '.$e->getMessage()]);
        }
        exit;
    }
    ...
    if ($method === 'POST' && $path === '/barber/appointments/walk-in') {
        ...
        if (! $customer_name || ! $customer_phone || ! $service_id) {
            http_response_code(400);
            echo json_encode(['error' => 'Customer name, phone, and service are required']);
            exit;
        }
        ...
        try {
            ...
            cc_notify_barber($pdo, (int) $barberId, [
                'sender_id' => $userId,
                'type' => 'booking',
            ...
            cc_notify_admin($pdo, [
                ...
                'message' => sprintf("Walk-in customer '%s' registered by barber. Payment: %s.", $customer_name, $payment_method),
                'related_entity_id' => (int) $appointmentId,
            ]);
            ...
            $pdo->rollBack();
            http_response_code(500);
            echo json_encode(['error' => 'Failed to create walk-in appointment: '.$e->getMessage()]);
        }
        exit;
    }
}
