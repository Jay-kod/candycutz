...
[$maxRequests, $windowSeconds] = $rateLimits[$rateLimitKey];
...
if ($method === 'GET') {
    ...
    if ($path === '/public/settings') {
        if (serveFromCache($cacheDir, 'public_settings', 300)) {
            exit;
        }
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            if (in_array($row['key'], $hiddenSettings, true)) {
                continue;
            }
    ...
    if ($path === '/public/services') {
        if (serveFromCache($cacheDir, 'public_services', 300)) {
            exit;
        }
        ...
        $barberMatchesService = function ($serviceName, $categoryId, $barber) use ($skillCategoryMap) {
            ...
            foreach ($specialties as $sp) {
                if (strtolower(trim((string) $sp)) === $needle) {
                    return true;
                }
            }
            foreach ($specialties as $sp) {
                $tag = strtolower(trim((string) $sp));
                if (isset($skillCategoryMap[$tag]) && (int) $skillCategoryMap[$tag] === (int) $categoryId) {
                    return true;
                }
            }
    ...
    if ($path === '/public/barbers') {
        if (serveFromCache($cacheDir, 'public_barbers', 300)) {
            exit;
        }
    ...
    if ($path === '/public/gallery') {
        if (serveFromCache($cacheDir, 'public_gallery', 300)) {
            exit;
        }
    ...
    if ($path === '/public/testimonials') {
        ...
        if (serveFromCache($cacheDir, $cacheKey, 300)) {
            exit;
        }
    ...
    if ($path === '/public/service-categories') {
        if (serveFromCache($cacheDir, 'public_categories', 300)) {
            exit;
        }
    ...
    // Blog posts
    if ($path === '/public/blog') {
        if (serveFromCache($cacheDir, 'public_blog', 300)) {
            exit;
        }
    ...
    if ($method === 'GET' && $path === '/notifications') {
        ...
        if ($role === 'customer') {
            ...
            foreach ($notifications as $n) {
                if ($n['type'] === 'booking' && ! $notifyBookings) {
                    continue;
                }
                if ($n['type'] === 'system_update' && ! $notifySystem) {
                    continue;
                }
                if ($n['type'] === 'wishlist_update' && ! $notifyWishlist) {
                    continue;
                }
                if ($n['type'] === 'blog_update' && ! $notifyBlog) {
                    continue;
                }
                if ($n['type'] === 'general_update' && ! $notifyGeneral) {
                    continue;
                }
