    ...
    if ($method === 'POST') {
        ...
        if ($username !== $user['username']) {
            if ($user['last_username_change'] !== null && strtotime($user['last_username_change']) > strtotime('-30 days')) {
                ...
                echo json_encode(['error' => 'Username can only be changed once every 30 days']);
                exit;
            }
            // Check uniqueness
        ...
        if ($email !== $user['email']) {
            if (! empty($user['email']) && $user['last_email_change'] && strtotime($user['last_email_change']) > strtotime('-30 days')) {
                http_response_code(400);
                echo json_encode(['error' => 'Email can only be changed once every 30 days']);
                exit;
            }
            // Check uniqueness
...
if ($method === 'PATCH' && preg_match('/^\/customer\/bookings\/(\d+)\/cancel$/', $path, $matches)) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        http_response_code(400);
        echo json_encode(['error' => 'Booking cannot be cancelled or not found']);
    }
    exit;
}
...
if ($method === 'DELETE' && preg_match('/^\/customer\/bookings\/(\d+)$/', $path, $matches)) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        http_response_code(400);
        echo json_encode(['error' => 'Booking cannot be deleted or not found']);
    }
    exit;
}
...
if ($method === 'POST' && $path === '/customer/testimonials') {
    ...
    if (! $comment || $rating < 1 || $rating > 5) {
        http_response_code(400);
        echo json_encode(['error' => 'Invalid rating or comment']);
        exit;
    }
...
if ($method === 'POST' && preg_match('/^\/customer\/checkout\/(\d+)\/receipt$/', $path, $matches)) {
    ...
    if (! isset($_FILES['receipt'])) {
        http_response_code(400);
        echo json_encode(['error' => 'No receipt uploaded']);
        exit;
    }
    ...
    if (! in_array($file['type'], $allowedTypes)) {
        http_response_code(400);
        echo json_encode(['error' => 'Invalid file type. Only JPG, PNG, and PDF allowed.']);
        exit;
    }
