...
export const Button: React.FC<ButtonProps> = ({
  ...
  const getContainerStyle = (): ViewStyle => {
    ...
    if (size === 'sm') {
      base = { ...base, paddingVertical: 8, paddingHorizontal: 12 };
    } else if (size === 'lg') {
  ...
  const getTextStyle = (): TextStyle => {
    ...
    if (size === 'sm') base.fontSize = FONTS.sizes.xs;
    else if (size === 'lg') base.fontSize = FONTS.sizes.lg;
    else base.fontSize = FONTS.sizes.sm;
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <>
          {icon && <>{icon}</>}
          <Text style={[getTextStyle(), icon ? { marginLeft: 6 } : null, textStyle]}>
            {title}
          </Text>
        </>
      )}
