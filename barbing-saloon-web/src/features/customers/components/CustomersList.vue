  ...
  <div>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      ...
      <div class="text-xs font-semibold text-white/30 tracking-wider uppercase">
        Total Base: <span class="text-white">{{ totalCustomers }}</span>
      </div>
    </div>
    ...
    <div class="rounded-[2rem] border border-white/[0.05] bg-[#1a1a1a]/80 backdrop-blur-2xl shadow-2xl overflow-hidden min-h-[400px]">
      ...
      <template v-else>
        <div v-if="filteredCustomers.length > 0" class="divide-y divide-white/[0.04]">
          ...
          >
            ...
            <div class="flex items-center justify-end gap-2 mt-3 md:mt-0">
              <button
                @click="$emit('viewProfile', customer)"
        ...
        <div v-else class="flex flex-col items-center justify-center py-24 px-6 text-center">
          ...
          <button v-if="search || currentFilter !== 'all'" @click="$emit('resetFilters')" class="mt-6 text-admin hover:text-admin-light text-sm font-semibold transition-colors">
            Clear filters
          </button>
        </div>
      </template>
...
<script setup>
import { 
  UserGroupIcon, CalendarDaysIcon, ChartBarIcon, DocumentTextIcon 
} from '@heroicons/vue/24/outline';

defineProps({
  loading: { type: Boolean, required: true },
  filteredCustomers: { type: Array, required: true },
...
defineEmits(['update:currentFilter', 'viewProfile', 'resetFilters']);
