  ...
  <SuperAdminLayout>
    <section class="space-y-8">
      <div class="rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] p-8">
        <p class="text-sm uppercase tracking-[0.3em] text-[var(--color-text-muted)]">Super Admin Dashboard</p>
        ...
        <div class="mt-6 space-y-3">
          <div v-for="log in dashboard.recent_logs || []" :key="log.id" class="flex items-center justify-between rounded-2xl border border-white/70 bg-white px-4 py-3">
            <div>
              ...
              <p class="text-xs text-[var(--color-text-muted)]">{{ log.action }} in {{ log.module }}</p>
            </div>
            <span class="text-xs text-[var(--color-text-muted)]">{{ log.created_at }}</span>
          </div>
        </div>
      </div>
    </section>
