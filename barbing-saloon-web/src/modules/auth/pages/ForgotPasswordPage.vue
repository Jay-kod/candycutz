    ...
    <div class="hidden lg:flex lg:w-1/2 relative flex-col items-center justify-between p-16 overflow-hidden text-center">
      ...
      <div class="relative z-10 max-w-lg">
        ...
        <h2 class="font-display text-5xl text-theme-text leading-tight mb-6">
          Lost access,<br />not lost style.
        </h2>
        <p class="text-lg text-theme-muted font-light leading-relaxed">
          Enter your email and we'll send you a link to get back into your CandyCutz account.
        </p>
      </div>
      ...
      <div class="w-full max-w-sm">
        <div class="mb-8">
          <div class="text-center">
            <h1 class="font-display font-bold text-3xl sm:text-4xl text-gold mb-4">Forgot Password</h1>
          </div>
          <p class="text-theme-muted text-xs sm:text-sm text-center">We'll email you a secure link to reset your password.</p>
        </div>
        ...
        <form class="space-y-4" @submit.prevent="submitForm">
          ...
          <div class="relative group">
            <label for="fp-email" class="block text-[10px] font-semibold uppercase tracking-wider text-gold mb-1.5">Email Address</label>
            <input
              v-model="email"
              id="fp-email"
...
async function submitForm() {
  ...
  try {
    ...
    message.value = response.data.message || 'If an account exists for this email, a reset link has been sent.';
  } catch (error) {
    if (error.inner) {
