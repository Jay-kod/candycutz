    ...
    <div class="hidden lg:flex lg:w-1/2 relative flex-col items-center justify-between p-16 overflow-hidden text-center">
      ...
      <div class="relative z-10 max-w-lg">
        ...
        <p class="text-lg text-theme-muted font-light leading-relaxed">
          Sign in to access the CandyCutz Admin Portal. Manage barbers, appointments, and overall shop operations.
        </p>
      </div>
      ...
      <div class="w-full max-w-sm">
        <div class="mb-8">
          <div class="text-center">
            <h1 class="font-display font-bold text-3xl sm:text-4xl text-admin mb-4">Admin Portal</h1>
          </div>
        ...
        <form class="space-y-4" @submit.prevent="submitForm">
          ...
          <div class="relative group">
            <label for="email" class="block text-[10px] font-semibold uppercase tracking-wider text-admin mb-1.5">Email Address</label>
            <input 
              v-model="email" 
              id="email"
          ...
          <!-- Password Input -->
          <div class="relative group">
            <label for="password" class="block text-[10px] font-semibold uppercase tracking-wider text-admin mb-1.5">Password</label>
            <div class="relative">
              <input 
                v-model="password" 
                id="password"
              ...
              <button 
                ...
                class="absolute right-4 top-1/2 -translate-y-1/2 text-neutral hover:text-admin transition-colors p-1">
                <!-- Eye Open -->
          ...
          <div class="flex items-center justify-between mt-2">
            <label class="flex items-center gap-2 cursor-pointer group">
              <div class="relative flex items-center justify-center w-4 h-4 rounded border border-theme-border bg-theme-bg group-hover:border-admin-dark transition-colors">
                <input type="checkbox" v-model="rememberMe" class="peer sr-only" />
                <svg class="w-3 h-3 text-admin opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                </svg>
              </div>
              <span class="text-xs text-theme-muted group-hover:text-admin-dark transition-colors">Remember me</span>
            </label>
            <RouterLink to="/forgot-password" class="text-xs font-semibold text-admin hover:text-admin-dark transition-colors">
              Forgot password?
            </RouterLink>
          </div>
          ...
          <div class="space-y-3 mt-6">
            ...
            <button 
              :disabled="loading" 
              class="relative w-full overflow-hidden rounded-lg bg-admin px-4 py-3 text-sm font-bold text-obsidian tracking-wide transition-all duration-300 hover:bg-admin-dark hover:shadow-[0_0_20px_rgba(255,103,0,0.3)] disabled:opacity-70 disabled:cursor-not-allowed group">
