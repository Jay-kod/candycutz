  ...
  <CustomerLayout>
    <section class="space-y-8 animate-fade-in">
      ...
      <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
        <article v-for="item in statsCards" :key="item.label" class="group relative overflow-hidden rounded-2xl border border-white/[0.06] bg-charcoal/80 p-6 backdrop-blur-sm transition-all hover:border-gold/20 hover:shadow-[0_0_20px_rgba(212,175,55,0.05)]">
          <div class="absolute right-0 top-0 -mr-4 -mt-4 opacity-5 transition-opacity group-hover:opacity-10">
            ...
            <span class="font-display text-9xl text-white">#</span>
          </div>
          <div class="relative z-10">
            <p class="text-xs font-semibold uppercase tracking-widest text-ivory/50">{{ item.label }}</p>
            <p class="mt-2 font-display text-4xl text-ivory group-hover:text-gold transition-colors">{{ item.value }}</p>
          </div>
        </article>
      </div>
      ...
      <div class="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <!-- Appointments -->
        <article class="rounded-2xl border border-white/[0.06] bg-charcoal/80 p-6 backdrop-blur-sm">
          <div class="flex items-center justify-between">
            <h2 class="font-display text-2xl text-ivory">Upcoming <span class="text-gold">Appointments</span></h2>
          ...
          <div class="mt-6 space-y-4">
            <div v-for="booking in dashboard.upcoming_appointments || []" :key="booking.id" class="rounded-xl border border-white/[0.04] bg-obsidian/50 p-5 transition-all hover:border-gold/30 hover:bg-white/[0.02]">
            ...
            <div v-if="(dashboard.upcoming_appointments || []).length === 0" class="rounded-xl border border-dashed border-white/10 p-8 text-center">
        ...
        <!-- Quick Links -->
        <article class="rounded-2xl border border-white/[0.06] bg-charcoal/80 p-6 backdrop-blur-sm">
          <h2 class="font-display text-2xl text-ivory">Quick <span class="text-gold">Links</span></h2>
          <div class="mt-6 space-y-3">
            <RouterLink to="/customer/dashboard/bookings" class="group flex items-center justify-between rounded-xl border border-white/[0.04] bg-obsidian/50 p-4 transition-all hover:border-gold/30 hover:bg-white/[0.02]">
              <div>
                <p class="text-sm font-semibold text-ivory group-hover:text-gold transition-colors">Bookings</p>
            ...
            <RouterLink to="/customer/dashboard/profile" class="group flex items-center justify-between rounded-xl border border-white/[0.04] bg-obsidian/50 p-4 transition-all hover:border-gold/30 hover:bg-white/[0.02]">
              <div>
                <p class="text-sm font-semibold text-ivory group-hover:text-gold transition-colors">Profile</p>
                <p class="mt-0.5 text-xs text-ivory/40">Update personal details & avatar</p>
              </div>
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-ivory/20 group-hover:text-gold transition-colors" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" /></svg>
            </RouterLink>
            <RouterLink to="/customer/dashboard/reviews" class="group flex items-center justify-between rounded-xl border border-white/[0.04] bg-obsidian/50 p-4 transition-all hover:border-gold/30 hover:bg-white/[0.02]">
              <div>
                <p class="text-sm font-semibold text-ivory group-hover:text-gold transition-colors">Reviews</p>
                <p class="mt-0.5 text-xs text-ivory/40">Leave or review feedback</p>
              </div>
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-ivory/20 group-hover:text-gold transition-colors" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" /></svg>
            </RouterLink>
            <RouterLink to="/customer/dashboard/wishlist" class="group flex items-center justify-between rounded-xl border border-white/[0.04] bg-obsidian/50 p-4 transition-all hover:border-gold/30 hover:bg-white/[0.02]">
              <div>
                <p class="text-sm font-semibold text-ivory group-hover:text-gold transition-colors">Wishlist</p>
