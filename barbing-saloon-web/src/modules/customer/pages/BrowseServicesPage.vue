    ...
    <section class="animate-fade-in space-y-8">
      ...
      <div class="flex flex-wrap gap-3">
        <button 
          ...
          :class="[
            ...
            activeCategory === 'All' ? 'bg-gold text-obsidian shadow-[0_0_15px_rgba(212,175,55,0.3)]' : 'bg-charcoal text-ivory/60 hover:text-gold hover:border-gold/50 border border-white/10'
          ]"
        >
          All
        </button>
        <button 
          ...
          :class="[
            'px-6 py-2.5 rounded-full text-sm font-semibold transition-all focus:outline-none',
            activeCategory === cat ? 'bg-gold text-obsidian shadow-[0_0_15px_rgba(212,175,55,0.3)]' : 'bg-charcoal text-ivory/60 hover:text-gold hover:border-gold/50 border border-white/10'
          ]"
        >
          {{ cat }}
        </button>
      ...
      <div v-if="isLoading" class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <div 
          v-for="n in 6" 
          :key="'skeleton-' + n" 
          class="relative flex flex-col overflow-hidden rounded-2xl border border-white/5 bg-charcoal/80"
      ...
      <div v-else-if="filteredServices.length > 0" class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <article 
          ...
          :key="service.id" 
          class="group relative flex flex-col overflow-hidden rounded-2xl border border-white/[0.06] bg-charcoal/80 backdrop-blur-sm transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-gold/5 hover:border-gold/20"
        >
          <!-- Image Container -->
          <div class="relative aspect-[4/3] w-full overflow-hidden bg-obsidian">
            ...
            />
            <div v-else class="h-full w-full flex items-center justify-center bg-obsidian">
            ...
            <!-- Category Badge -->
            <span class="absolute left-4 top-4 rounded-full bg-obsidian/90 backdrop-blur px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-gold shadow-sm border border-gold/20">
            ...
            <button 
              ...
              class="absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-full bg-obsidian/90 backdrop-blur text-ivory border border-white/10 shadow-sm transition-all hover:bg-obsidian hover:scale-110 focus:outline-none"
          ...
          <div class="flex flex-1 flex-col p-6">
            <h2 class="font-display text-xl font-bold text-ivory group-hover:text-gold transition-colors">{{ service.name }}</h2>
            ...
            <div class="mt-6 flex items-center justify-between border-t border-white/10 pt-5">
              <div class="flex flex-col">
                ...
                <span class="font-display text-lg font-bold text-ivory mt-0.5">₦{{ Number(service.price).toLocaleString() }}</span>
              </div>
      ...
      <!-- Empty State -->
      <div v-else class="rounded-2xl border border-white/[0.06] bg-charcoal/80 py-24 text-center backdrop-blur-sm">
        ...
        <h3 class="text-xl font-display text-ivory mb-2">No services found</h3>
