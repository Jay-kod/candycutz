    ...
    <section class="animate-fade-in space-y-8">
      ...
      <div class="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <form class="rounded-2xl border border-white/[0.06] bg-charcoal/80 p-8 backdrop-blur-sm" @submit.prevent="submit">
          <h2 class="font-display text-2xl text-ivory">Personal Info</h2>
          <div class="mt-6 space-y-5">
            <div class="space-y-2">
              <label class="text-xs font-semibold uppercase tracking-widest text-ivory/50">Name</label>
              <input v-model="form.name" class="w-full rounded-xl border border-white/[0.08] bg-obsidian px-4 py-3 text-ivory placeholder-ivory/30 outline-none transition-colors focus:border-gold/50" />
            </div>
            <div class="space-y-2">
              <label class="text-xs font-semibold uppercase tracking-widest text-ivory/50">Phone</label>
              <input v-model="form.phone" class="w-full rounded-xl border border-white/[0.08] bg-obsidian px-4 py-3 text-ivory placeholder-ivory/30 outline-none transition-colors focus:border-gold/50" />
            </div>
            <div class="space-y-2">
              <label class="text-xs font-semibold uppercase tracking-widest text-ivory/50">Avatar</label>
              <input type="file" @change="handleFile" class="w-full rounded-xl border border-white/[0.08] bg-obsidian px-4 py-3 text-sm text-ivory file:mr-4 file:rounded-full file:border-0 file:bg-gold/10 file:px-4 file:py-2 file:text-xs file:font-bold file:text-gold hover:file:bg-gold/20 outline-none transition-colors" />
            </div>
          </div>
        ...
        <div class="rounded-2xl border border-white/[0.06] bg-charcoal/80 p-8 backdrop-blur-sm">
          <h2 class="font-display text-2xl text-ivory">Recent <span class="text-gold">Activity</span></h2>
          <div class="mt-6 space-y-4">
            <article v-for="booking in history.latest_bookings || []" :key="booking.id" class="rounded-xl border border-white/[0.04] bg-obsidian/50 p-5 transition-all hover:border-gold/30 hover:bg-white/[0.02]">
            ...
            <div v-if="(history.latest_bookings || []).length === 0" class="rounded-xl border border-dashed border-white/10 p-8 text-center">
