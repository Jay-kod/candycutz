  ...
  <CustomerLayout>
    <main class="min-h-screen bg-theme-bg pb-32">
      ...
      <div v-else-if="error" class="mx-auto max-w-2xl px-6 text-center py-40">
        ...
        <RouterLink to="/customer/dashboard/services" class="mt-10 inline-block rounded-full bg-gold px-8 py-4 text-obsidian font-bold tracking-wide hover:bg-gold-light transition-all">Back to Services</RouterLink>
      </div>
      ...
      <div v-else>
        ...
        <section class="relative h-[60vh] min-h-[500px] w-full overflow-hidden">
          <div class="absolute inset-0">
            <img v-if="barber.avatar" :src="getFullImageUrl(barber.avatar)" :alt="barber.name" class="h-full w-full object-cover scale-105" />
          ...
          <div class="grid gap-16 lg:grid-cols-12 lg:items-start">
            ...
            <div class="lg:col-span-5 lg:sticky lg:top-32" data-reveal data-reveal-delay="200">
              <div class="rounded-3xl border border-theme-border bg-theme-surface p-8 shadow-2xl relative overflow-hidden">
                ...
                <RouterLink to="/customer/dashboard/services" class="group flex w-full items-center justify-center gap-3 rounded-2xl bg-gold py-5 text-lg font-bold text-obsidian shadow-[0_0_40px_-10px_rgba(212,175,55,0.5)] hover:shadow-[0_0_60px_-10px_rgba(212,175,55,0.7)] transition-all duration-300 hover:-translate-y-1">
                  Browse Services
