    ...
    <section class="animate-fade-in pb-10">
      ...
      <div class="relative overflow-hidden rounded-[2rem] border border-white/[0.05] bg-[#111111]/90 p-8 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 backdrop-blur-3xl mb-8">
        ...
        <div class="relative z-10 flex items-center gap-6">
          <router-link to="/admin/services" class="group flex h-12 w-12 items-center justify-center rounded-2xl bg-white/5 border border-white/10 text-white/50 hover:text-white hover:bg-white/10 transition-all hover:shadow-[0_0_20px_rgba(255,103,0,0.15)] hover:border-admin/30">
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-8 items-start">
        ...
        <div class="space-y-8">
          ...
          <div class="md:hidden flex gap-4 pt-4">
            <router-link to="/admin/services" class="flex-[1] flex items-center justify-center py-4 px-4 rounded-2xl border border-white/10 text-sm font-bold text-white/60 hover:text-white hover:bg-white/5 transition-all">
              Cancel
            </router-link>
...
async function loadData() {
  ...
  try {
    try {
      const catRes = await adminApi.serviceCategories();
...
async function submitForm() {
  ...
  try {
    ...
    if (isEditing.value) {
      await adminApi.updateService(serviceId.value, payload);
