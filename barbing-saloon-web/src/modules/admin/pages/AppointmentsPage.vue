  ...
  <AdminLayout>
    <section class="space-y-6">
      <div class="rounded-3xl border border-theme-border bg-theme-surface p-8">
        <p class="text-sm uppercase tracking-[0.3em] text-theme-muted">Appointments</p>
        <h1 class="mt-3 font-display text-4xl text-admin">Manage bookings</h1>
      </div>

      <div class="grid gap-4">
        <article v-for="appointment in appointments.data || appointments" :key="appointment.id" class="rounded-3xl border border-theme-border bg-theme-surface p-6">
          <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
              <p class="font-display text-2xl text-obsidian">{{ appointment.client_name }}</p>
              <p class="text-sm text-theme-muted">{{ appointment.service?.name }} with {{ appointment.barber?.name }} on {{ appointment.appointment_date }} at {{ appointment.appointment_time }}</p>
            </div>
            <div class="flex gap-3 text-xs font-semibold">
              ...
              <button class="rounded-full border border-theme-border px-3 py-1" @click="cancel(appointment.id)">Cancel</button>
            </div>
          </div>
        </article>
      </div>
